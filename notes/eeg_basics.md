...
Electroencephalography or EEG is a method for recording electrical brain activity from electrodes placed on the scalp. EEG does not measure individual neurons directly. Instead, it records large-scale electrical activity generated by many neurons acting together.[^cohen-notes]
...
EEG is difficult to interpret because scalp electrodes measure mixtures of underlying sources. These sources can include neural activity from different brain regions, as well as non-neural noise such as eye movements, muscle activity, and electrical artifacts.[^cohen-notes]

The EEG signals contain multiple true underlying sources, as brainwaves from multiple regions can be picked up by one electrode. This creates noise.[^cohen-notes] 

EEG also has limited anatomical precision. Activity from different sources can overlap, and opposing electrical fields can cancel each other out. This makes it difficult to say exactly where a scalp-level signal came from.

Filtering can help isolate frequency ranges of interest, but it does not perfectly separate signal from noise. Filtering is a tool for making parts of the signal easier to study, not a complete solution to the source-separation problem.
